...
\item{\selection{match\_ecdfs}: updated matching to high-quality redshift datalogs in the ECDFS field, also requiring high signal to noise photometry, with $SN_{I} > 20$, see Sec.~\ref{sec:data:reference}.}
...
\begin{tabular}{lll}
 ...
 training\_v4 & \selection{match\_ecdfs}  & 6,778\\
 test\_v4 & \selection{match\_ecdfs}  & 2,905 \\
...
We applied conservative cuts for our fiducial analyses, specifically \code{type == ''s''}, \code{confidence >= 0.90} SNR $>= 20$ in the $i$ band (using \code{gaap1p0} fluxes), and requiring measured Gaussian aperture (\code{gaap\_1p0}) fluxes in all bands.  
We then performed a random 70\%/30\% train/test split on this catalog, resulting in a training set of  6,778 redshifts and a test set of 2,905 redshifts.
...
\begin{table}[!p]
    ...
    \caption{
        ...
        Specifically, \code{type == ''s''}, \code{confidence >= 0.90}, SNR $>= 20$ in the $i$ band (using \code{gaap1p0} fluxes).\\
        $^{*}$ Note: multiband photo-z redshifts and grism and spectroscopic redshifts with confidence < 0.90 were not used in any training sets employed in this note.
    }
...
This selection also dropped all galaxies with unmeasured magnitude in any of the six bands.
...
we have updated the note to use the \dataset{train\_v4} and \dataset{test\_v4} datasets.
...
Specifically, we used \code{rail\_project} to run all of the configuration flavors defined in the \href{https://github.com/LSSTDESC/rail_project_config/blob/main/dp1/dp1.yaml}{dp1/dp1.yaml} and \href{https://github.com/LSSTDESC/rail_project_config/blob/main/dp1/dp1_v4.yaml}{dp1/dp1\_v4.yaml} configuration files.
...
\hline
FlexZBoost & 0.000 (0.000) & 0.026 (0.028) & 0.192 (0.214) & 0.093 (0.105) \\
kNN        & -0.003 (-0.000) & 0.035 (0.031) & 0.194 (0.220) & 0.119 (0.125) \\
CMNN       & 0.000 (-0.002) & 0.039 (0.073) & 0.172 (0.171) & 0.121 (0.186) \\
DNF        & -0.003 (-0.000) & 0.048 (0.034) & 0.143 (0.189) & 0.109 (0.107) \\
TPZ        & -0.001 (-0.003) & 0.054 (0.056) & 0.126 (0.133) & 0.104 (0.111) \\
GPz        & 0.003 (0.002) & 0.052 (0.060) & 0.181 (0.159) & 0.147 (0.144) \\
BPZ        & -0.018 (-0.016) & 0.039 (0.055) & 0.221 (0.223) & 0.168 (0.202) \\
LePhare    & -0.014 (-0.015) & 0.034 (0.066) & 0.208 (0.178) & 0.147 (0.175) \\
\hline
\end{tabular}
...
Tab.~\ref{tab:tech_perf} shows compute times and file sizes for both the training and estimation phases of the analysis.    The metrics from the training phase were taken from running the algorithm's ``Inform'' stages on \href{https://docs.nersc.gov/systems/perlmutter/architecture}{Perlmutter at NERSC} on the ``training\_v4'' dataset of 6,778 objects.    The metrics from the estimation phase were taken from algorithm's ``Inform'' stages on USDF on the ``test\_v4'' datasets of 2,905 objects.
...
\end{tabular}
\caption{
  Algorithm compute times and files sizes.   The algorithms were trained on 6,778 objects and evaluated on 2,905 objects.  The variations shown reflect the differences in processing times with different sets of hyper-parameters.
}
\label{tab:tech_perf}
\end{table*}
...
%  LocalWords:  Lephare _bpz Ilbert:09 _bpz knn bpz
%  LocalWords:  rightarrow rightarrow knn bpz textbf textbf textbf
%  LocalWords:  Sersic _ecdfs _psfFlux _psfFluxErr 2dflens _ecfds psf
%  LocalWords:  _pz
