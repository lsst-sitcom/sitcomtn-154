...
The \selection{match\_prelim} selection used in the \dataset{train\_v1} and \dataset{test\_v1} datasets did not use the confidence cut, and only required SNR $>= 5$ in the $i$ band (using \code{psf} fluxes).   We randomly selection 7,000 objects for the \dataset{train\_v1} and used the remaining 2,438 for the \dataset{test\_v1}  dataset.    Although we used these datasets extensively in V1 of this technote, we intend to update the note to use the \dataset{train\_v2} and \dataset{test\_v2} as soon as possible, ideally in the first week of July, 2025.
